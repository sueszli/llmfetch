...
for i in {1..30}; do
    curl -s "http://localhost:3000/jobs" > /dev/null 2>&1 && break
...
RESPONSE=$(curl -s "http://localhost:3000/jobs")
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
[ "$RESPONSE" = "[]" ] && pass || fail

test "create and scrape job"
RESPONSE=$(curl -s -X POST "http://localhost:3000/jobs" -H "Content-Type: application/json" -d '{"url":"https://example.com","fields":["title","heading"]}')
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
...
RESPONSE=$(curl -s "http://localhost:3000/jobs/$ID")
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
...
test "delete job"
RESPONSE=$(curl -s -X DELETE "http://localhost:3000/jobs/$ID")
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
...
RESPONSE=$(curl -s "http://localhost:3000/jobs/invalid")
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
echo "$RESPONSE" | grep -q '"error"' && pass || fail

test "error: missing url and fields"
RESPONSE=$(curl -s -X POST "http://localhost:3000/jobs" -H "Content-Type: application/json" -d '{}')
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
echo "$RESPONSE" | grep -q '"error"' && pass || fail

test "error: missing url"
RESPONSE=$(curl -s -X POST "http://localhost:3000/jobs" -H "Content-Type: application/json" -d '{"fields":["title"]}')
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
echo "$RESPONSE" | grep -q '"error"' && pass || fail

test "error: missing fields"
RESPONSE=$(curl -s -X POST "http://localhost:3000/jobs" -H "Content-Type: application/json" -d '{"url":"https://example.com"}')
echo -e "\033[0;33mresponse:\033[0m $RESPONSE"
echo "$RESPONSE" | grep -q '"error"' && pass || fail
